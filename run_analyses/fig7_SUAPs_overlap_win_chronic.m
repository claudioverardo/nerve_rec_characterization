%% PLOT FIGURES 7A-B
% the data must be simulated with the script run_simulations/sim6_SUAPs_along_fiber_chronic.m
% refer to that script for the nomenclature used to store simulated data
DIR_DATA = ""; % TO SET
dir_experiments = fullfile(DIR_DATA, "SUAPs_analysis_along_fiber_chronic");


%% PLOT OVERLAP WINDOW OF SUAP AMPLITUDES (CHRONIC SETTINGS)

d_fibers = [2:2:14];
d_fibers_unmyel = [0.1 0.5 1.0 1.5 2.0];
colors_diams = lines();
% colors_diams(3:4,:) = [];

% uncomment one of the code blocks below to plot the desired figures
% =============================================================================================================================
% INTRAFASCICULAR (TIME) w/ 25-50-100 um encapsulation thickness
elec = "TIME";
elec_mesh = 1;
T_fibrosis = [25 50 100];
y_el = -2;
Vpp_lim_violins = [0 75];
dz = 5; % um
median_colors = [];
fn_opts = @(elec, elec_mesh, d_fiber, z_shift, y_el, T_fibrosis) ...
    {elec   elec_mesh      4       200     0           nan        "ascent_mrg"      d_fiber      0+T_fibrosis   z_shift    y_el    T_fibrosis};
fn_opts_unmyel = @(elec, elec_mesh, d_fiber, z_shift, y_el, T_fibrosis) ...
    {elec   elec_mesh      4       200     0           nan        "ascent_sundt15"  d_fiber      0+T_fibrosis   z_shift    y_el    T_fibrosis};
% =============================================================================================================================AL
% % EXTRANEURAL (cuff) w/ 100-250-500 um encapsulation thickness
% elec = "Cortec_Cuff";
% elec_mesh = 2;
% T_fibrosis = [100 250 500];
% y_el = nan; % NOTE: not needed!
% d_as_fasc = 20;
% d_fasc_fib = 200;
...
% fn_opts = @(elec, elec_mesh, d_fiber, z_shift, y_el, T_fibrosis) ...
%     {elec   elec_mesh      4       200     -800+d_as_fasc        nan        "ascent_mrg"       d_fiber      -1000+d_fasc_fib+d_as_fasc      z_shift     nan    T_fibrosis};
% fn_opts_unmyel = @(elec, elec_mesh, d_fiber, z_shift, y_el, T_fibrosis) ...
%     {elec   elec_mesh      4       200     -800+d_as_fasc        nan        "ascent_sundt15"    d_fiber      -1000+d_fasc_fib+d_as_fasc     z_shift     nan    T_fibrosis};
%===========================================================================


% myelinated fibers
z_shift = cell(length(d_fibers), 1);
N1 = cell(length(d_fibers), length(T_fibrosis));
TN1 = cell(length(d_fibers), length(T_fibrosis));
P1 = cell(length(d_fibers), length(T_fibrosis));
TP1 = cell(length(d_fibers), length(T_fibrosis));
Vpp = cell(length(d_fibers), length(T_fibrosis));
Tpp = cell(length(d_fibers), length(T_fibrosis));
for i=1:length(d_fibers)
    d_fiber = d_fibers(i);
    [z_shift{i}, z_shift_m] = dz_to_zshift("ascent_mrg", d_fiber, dz);

    for j=1:length(T_fibrosis)
        opts = fn_opts(elec, elec_mesh, d_fiber, z_shift{i}, y_el, T_fibrosis(j));
...
% unmyelinated fibers
N1_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
TN1_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
P1_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
TP1_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
Vpp_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
Tpp_unmyel = cell(length(d_fibers_unmyel), length(T_fibrosis));
for i=1:length(d_fibers_unmyel)
    d_fiber = d_fibers_unmyel(i);
    z_shift_unmyel = [0; 0.5];
    
    for j=1:length(T_fibrosis)
        opts = fn_opts_unmyel(elec, elec_mesh, d_fiber, z_shift_unmyel, y_el, T_fibrosis(j));
        dir_model = fullfile(dir_experiments, DataHash(opts, 'MD5'));
