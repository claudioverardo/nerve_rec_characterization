%% SIMULATE DATA USED IN FIGURE 7
DIR_DATA = ""; % TO SET
...
% opts_i{12}    T_fibrosis  [um]    thickness of the fibrotic tissue
% ---------------------------------------------------------------------------------------------------------------
...
for i=1:length(d_fibers)
    ...
    for j=1:length(T_fibrosis)
        opts{end+1} = {"TIME"   1      4       200     0       nan         "ascent_mrg"         d_fiber         0+T_fibrosis(j)     z_shift     y_el     T_fibrosis(j)};
    end
end
% unmyelinated fibers
...
for i=1:length(d_fibers)
    d_fiber = d_fibers(i);
    for j=1:length(T_fibrosis)
        opts{end+1} = {"TIME"   1      4       200     0       nan         "ascent_sundt15"     d_fiber         0+T_fibrosis(j)     z_shift     y_el     T_fibrosis(j)};
    end
end
% =============================================================================================================================
...
for i=1:length(d_fibers)
    ...
    [z_shift, z_shift_m] = dz_to_zshift("ascent_mrg", d_fiber, dz);
    for j=1:length(T_fibrosis)
        opts{end+1} = {"Cortec_Cuff"    2      4       200     -800+d_as_fasc       nan         "ascent_mrg"         d_fiber         -1000+d_fasc_fib+d_as_fasc       z_shift     nan     T_fibrosis(j)};
    end
end
% unmyelinated fibers
...
for i=1:length(d_fibers)
    d_fiber = d_fibers(i);
    for j=1:length(T_fibrosis)
        opts{end+1} = {"Cortec_Cuff"     2      4       200     -800+d_as_fasc       nan         "ascent_sundt15"    d_fiber         -1000+d_fasc_fib+d_as_fasc       z_shift     nan     T_fibrosis(j)};
    end
end
% =============================================================================================================================
    ...
    if ~(isfile(fullfile(hm_params.dir_model, "hm_model.mat")) && isfile(fullfile(hm_params.dir_model, "SUAP_1_1.mat")))
        ...
        switch hm_params.implant_type
            ...
            case 'Cortec_Cuff'
                ...
                hm_params.electrodes.radius = R_nerve + opts_i{12}*1e-6;
                ...
                hm_params.electrodes.length_as = 2*pi*hm_params.electrodes.radius;
                ...
                hm_params.electrodes.thick_encaps = opts_i{12}*1e-6;
